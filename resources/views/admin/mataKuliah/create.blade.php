...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.mataKuliah.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.mata-kuliah.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sks') ? 'is-invalid' : '' }}" type="text" name="sks" id="sks" value="{{ old('sks', '') }}" required>
                @if($errors->has('sks'))
                    <div class="invalid-feedback">
                        {{ $errors->first('sks') }}
                    </div>
