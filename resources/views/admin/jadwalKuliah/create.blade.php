...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.jadwalKuliah.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.jadwal-kuliah.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mataKuliah') ? 'is-invalid' : '' }}" name="mata_kuliah_id" id="mata_kuliah_id" required>
                    @foreach($mataKuliah as $id => $mataKuliah)
                        <option value="{{ $id }}" {{ old('mata_kuliah_id') == $mataKuliah ? 'selected' : '' }}>{{ $mataKuliah }}</option>
                    @endforeach
                </select>
